...
if [ "$DOCKER_BUILD" = true ] || [ "$DOCKER_COMPOSE_UP" = true ]; then
    ...
    if [ $? -eq 0 ]; then
        ...
        echo "  docker run -p 8081:8081 ${IMAGE_NAME}:${IMAGE_TAG}"
        echo ""
        echo "To run with environment variables:"
        echo "  docker run -p 8081:8081 -e DB_MOCK_MODE=false -e DB_URL=... ${IMAGE_NAME}:${IMAGE_TAG}"
        echo ""
    else
...
if [ "$DOCKER_COMPOSE_UP" = true ]; then
    ...
    if [ $? -eq 0 ]; then
        echo ""
        echo "================================================"
        echo "✅ Helidon MP Application Started Successfully!"
        echo "================================================"
        echo ""
        echo "🏠 Home Page (index.html):"
        echo "   http://localhost:8081/hostinfo/"
        echo ""
        echo "📡 REST API Endpoints:"
        echo "   GET  http://localhost:8081/hostinfo/api/host-info              - Host Information"
        echo "   GET  http://localhost:8081/hostinfo/api/database-info          - Database Status"
        echo "   GET  http://localhost:8081/hostinfo/api/database-info/test     - Test DB Connection"
        echo "   GET  http://localhost:8081/hostinfo/api/session-info           - Get/Create Session"
        echo "   GET  http://localhost:8081/hostinfo/api/session-info/{id}      - Get Session by ID"
        echo "   PUT  http://localhost:8081/hostinfo/api/session-info/{id}/attribute  - Set Attribute"
        echo "   POST http://localhost:8081/hostinfo/api/session-info/{id}/user - Set Username"
        echo "   DEL  http://localhost:8081/hostinfo/api/session-info/{id}      - Invalidate Session"
        echo "   GET  http://localhost:8081/hostinfo/api/greet?name=X           - Greeting"
        echo "   GET  http://localhost:8081/hostinfo/api/greet/welcome          - Welcome Message"
        echo "   GET  http://localhost:8081/hostinfo/api/greet/info             - Service Info"
        echo ""
        echo "📚 Documentation & Health:"
        echo "   http://localhost:8081/hostinfo/openapi                         - OpenAPI Spec (YAML)"
        echo "   http://localhost:8081/health                                   - Health Check"
        echo "   http://localhost:8081/health/ready                             - Readiness"
        echo "   http://localhost:8081/health/live                              - Liveness"
        echo ""
        echo "🔧 Management:"
...
if [ "$RUN_APP" = true ]; then
    ...
    echo "  http://localhost:8081/api/host-info"
    echo ""
    echo "OpenAPI documentation:"
    echo "  http://localhost:8081/openapi"
    echo ""
    echo "Health check:"
